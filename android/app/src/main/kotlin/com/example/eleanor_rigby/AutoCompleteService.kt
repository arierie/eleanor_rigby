...
interface AutoCompleteService {
    ...
    sealed interface AutoCompleteResult {
        data class Success(val words: String) : AutoCompleteResult
...
class AutoCompleteServiceImpl constructor(
     ...
     * @return [InitModelResult.Success] if TFLite was initialized properly, otherwise [InitModelResult.Error]
     */
    override suspend fun initModel(): AutoCompleteService.InitModelResult {
        return withContext(dispatcher) {
            ...
            if (loadResult.isFailure) {
                ...
                return@withContext if (exc is FileNotFoundException) {
                    AutoCompleteService.InitModelResult.Error(AutoCompleteService.AutoCompleteServiceError.MODEL_FILE_NOT_FOUND)
                } else {
                    AutoCompleteService.InitModelResult.Error(AutoCompleteService.AutoCompleteServiceError.MODEL_NOT_INITIALIZED)
                }
            }
            ...
            if (isInitialized) AutoCompleteService.InitModelResult.Success
            else AutoCompleteService.InitModelResult.Error(AutoCompleteService.AutoCompleteServiceError.MODEL_NOT_INITIALIZED)
        }
    }
    ...
    override suspend fun getSuggestion(input: String, applyWindow: Boolean, windowSize: Int) = withContext(dispatcher) {
        ...
        if (!::interpreter.isInitialized) {
            ...
            if (result is AutoCompleteService.InitModelResult.Error) {
                return@withContext AutoCompleteService.AutoCompleteResult.Error(result.error)
            }
        }
        ...
        if (containsBadLanguage) {
            ...
            return@withContext AutoCompleteService.AutoCompleteResult.Error(AutoCompleteService.AutoCompleteServiceError.NO_SUGGESTIONS)
        }
        ...
        if (output.length < trimmedInput.length) {
            ...
            AutoCompleteService.AutoCompleteResult.Error(AutoCompleteService.AutoCompleteServiceError.NO_SUGGESTIONS)
        } else {
            ...
            if (newText.isEmpty()) {
                ...
                Napier.d { "[6] New text split in words: (${newText.length} words)" }
                AutoCompleteService.AutoCompleteResult.Success(newText)
            }
        }
    }
    ...
    companion object {
        ...
        private const val TFLITE_MODEL = "eleanor_rigby.tflite"
